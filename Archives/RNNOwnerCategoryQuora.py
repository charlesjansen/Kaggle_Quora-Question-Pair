...
features_1_2 = np.concatenate((features1, np.zeros((len(features1), 5), dtype=int), features2), axis=1)
features_2_1 = np.concatenate((features2, np.zeros((len(features1), 5), dtype=int), features1), axis=1)
...
preds1_2 = model.predict(features_1_2, batch_size=128, verbose=1)
preds2_1 = model.predict(features_2_1, batch_size=128, verbose=1)
predsMerge = (preds1_2 + preds2_1) / 2

submission = pd.DataFrame({'test_id':test_ids, 'is_duplicate':preds.ravel()})
submission.to_csv('rnnStyle1 20170508 matin.csv', index=False)
